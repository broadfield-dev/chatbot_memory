...
class MemoryManager:
    def __init__(self, long_term_backend, max_short_term_size=50):
    ...
    def process_content(self, source, content, truthfulness=0.5, importance=0.5):
        """Add content to short-term memory and consolidate to long-term."""
        timestamp = datetime.now().isoformat()
        self.short_term_collection.add(
            ids=[f"{source}_{timestamp}"],
            documents=[content],
            metadatas={"type": source, "timestamp": timestamp, "truthfulness": truthfulness, "importance": importance}
        )
        self.consolidate_memory(content, truthfulness, importance)
